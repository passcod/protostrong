const { $debug, $info } = require('./logger')('paths')
const { promises: fs, constants: { R_OK: READABLE } } = require('fs')
...
async function check (file) {
  ...
  try {
    await fs.access(file, READABLE)
    return true
  } catch (e) {
    $debug('fs call on', file, 'errored')
    $debug(e)
    return false
  }
}
